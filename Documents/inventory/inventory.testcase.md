...
#### 测试用例2.4.4：库存列表操作菜单国际化（新增 2026-02-09）
- **前置条件**：
  - 用户已登录（BOSS/STAFF）
  - 在库存列表页面
  - 存在可用的库存商品
...
- **预期结果**：
  - 中文模式下：
    - 操作菜单文案为“录入损耗 / 编辑 / 查看变更历史”（或项目实际提供的菜单项）
  - 英文模式下：
    - 操作菜单文案为“Record Loss / Edit / View Change History”（或项目实际提供的菜单项）

#### 测试用例2.4.5：库存列表录入损耗弹窗国际化（新增 2026-02-09）
- **前置条件**：
  - 用户已登录（BOSS/STAFF）
  - 在库存列表页面
  - 通过操作菜单进入“录入损耗”弹窗
...
### 变更5：进货单与库存列表滚动支持（2026-02-09）

**变更内容**：
- 新增测试用例1.5.1：进货单列表y轴滚动
- 新增测试用例2.6.1：库存列表y轴滚动

**变更原因**：
- 支持进货单管理与库存管理页面在数据量较大时的垂直滚动体验
