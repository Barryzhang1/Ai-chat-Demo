...
3. **验证座位状态（前端控制）**
   - **重要**：排队中的用户无法创建订单
   - 前端通过 `queueInfo` 和 `seatInfo` 状态判断用户是否已分配座位
   - 如果用户处于排队状态（`queueInfo` 存在且 `seatInfo` 为空），支付按钮将被禁用
   - 用户必须等待座位分配后才能支付创建订单
   - 后端不强制验证座位状态，主要依赖前端控制
...
```
用户点击下单 → 前端验证座位状态 → 验证认证 → 获取购物车 → 验证购物车 → 生成订单 → 保存订单 → 清空购物车和查询条件 → 返回订单详情
                ↓ (排队中)         ↓           ↓            ↓                                                     ↓
              禁用按钮+提示      401错误     404错误      400错误                                          启动新会话
```
...
2. **订单状态流转**：添加商家接单、制作中、完成等状态
3. **订单取消**：允许用户取消未开始制作的订单
4. **订单评价**：订单完成后允许用户评价菜品

## 排队用户限制 (2026-02-04 新增)

### 功能说明

为了保证就餐流程的合理性，**排队中的用户无法创建订单**。用户必须等待座位分配后才能支付下单。

### 实现方式

#### 前端控制（主要）

**文件位置**：[ChatUI/src/pages/UserOrder/UserOrder.js](../../ChatUI/src/pages/UserOrder/UserOrder.js)

**判断逻辑**：
```javascript
// 通过 queueInfo 和 seatInfo 判断用户状态
const isInQueue = queueInfo && !seatInfo;

// 支付按钮状态
disabled={isInQueue}
color={isInQueue ? 'default' : 'success'}
```
...
2. **支付按钮禁用**：
   - AI推荐菜单卡片：支付按钮变为灰色且禁用
   - 菜单Popup：支付按钮显示"支付（排队中）"且禁用
...
4. **其他功能不受影响**：
   - 用户仍可以浏览菜单、添加购物车、刷新推荐
   - 只有支付功能被限制

#### 后端说明（辅助）

**文件位置**：[ChatBackEnd/src/modules/ordering/ordering.service.ts](../../ChatBackEnd/src/modules/ordering/ordering.service.ts)

由于座位系统使用 `socketId` 而不是 `userId` 进行用户标识，后端难以直接验证用户的座位状态。因此：
- 后端不强制验证座位状态
- 主要依赖前端控制防止排队用户创建订单
- 这种设计简化了系统复杂度，同时保证了功能的正确性

### 用户体验流程

```
用户进入点餐 → 座位已满 → 进入排队 → 可以浏览和添加购物车
     ↓
排队等待 → 支付按钮禁用 → 提示等待座位分配
     ↓
获得座位 → 收到通知 → 支付按钮启用 → 可以创建订单
```

### 相关文档

- 测试用例：[queue-user-order-restriction.testcase.md](./queue-user-order-restriction.testcase.md)
- 座位管理：[../seat/SEAT_COMPLETE_GUIDE.md](../seat/SEAT_COMPLETE_GUIDE.md)
